...
def estimate_midlane(midlane_patches,Max_dist):
    ...
    cnts, _ = cv2.findContours(midlane_patches,cv2.RETR_LIST,cv2.CHAIN_APPROX_SIMPLE)
    ...
    for index, cnt in enumerate(cnts):
        ...
        if (type(BstCentroid_a)!= int):
            ...
            ab_line.append([BstCentroid_a, BstCentroid_b])
    ...
    if ab_line:
        ...
        for line in ab_line:
            ...
            if dist > max_distance :
                ...
                point1_max = point1
                point2_max = point2
        if not (point2_max[0] <  midlane_patches.shape[0]/3 or point2_max[0] <  midlane_patches.shape[0]/3) :  
            if point2_max[0] - point1_max[0] != 0 and (point2_max[1] - point1_max[1]) != 0 :  # Kiểm tra xem mẫu số có khác 0 không
                slope = (point2_max[1] - point1_max[1]) / (point2_max[0] - point1_max[0])
                y_intercept = point1_max[1] - slope * point1_max[0]
                bottom_y = midlane_patches.shape[0]
                intersection_x = int((bottom_y - y_intercept) / slope)

                if point1_max > point2_max:
                    point = point1_max
                else:
                    point =point2_max
                    
                if (intersection_x -point1[1]) <  midlane_patches.shape[0]/4 : 
                    if intersection_x < 320 and intersection_x > 0:
                        midlane_connectivity_bgr = cv2.line(midlane_connectivity_bgr, point, (intersection_x, bottom_y), (0, 0, 255), 2)
